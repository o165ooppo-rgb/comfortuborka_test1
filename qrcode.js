...
(function (factory) {
  if (typeof define === 'function' && define.amd) {
      ...
      // Браузер без модульной системы — вешаем на window
      if (typeof window !== 'undefined') { window.qrcode = factory(); }
  }
}(function () {
    return qrcode;
}));
...
/* =========================================================
   KusQR — обёртка над qrcode-generator (Kazuhiko Arase, MIT)
   ---------------------------------------------------------
   Генерирует QR офлайн и рендерит в PNG Data URL.
   window.KusQR.toDataURL(text, { size, margin, dark, light })
========================================================= */
(function (global) {
  'use strict';
  function getQrFactory() {
    if (typeof global.qrcode === "function") return global.qrcode;
  ...
  function toDataURL(text, opts) {
    opts = opts || {};
    var pxSize = opts.size || 220;
    var margin = opts.margin != null ? opts.margin : 2;
    var dark = opts.dark || "#0a1230";
    var light = opts.light || "#ffffff";

    // type 0 = авто-подбор версии, 'M' = средний уровень коррекции
    var factory = getQrFactory();
    var qr = factory(0, 'M');
    qr.addData(String(text));
    qr.make();
    var n = qr.getModuleCount();
    var total = n + margin * 2;
    var scale = Math.max(1, Math.floor(pxSize / total));
    var canvasSize = total * scale;
